...
namespace subrunner.goap
{
    public class WaitTargetSensor : LocalTargetSensorBase, LoadedSensor
    {
        ...
        {
            if (Logger.LazyInstance.LOG_WANDER_TARGET_SENSOR) { Debug.Log($"(WaitSensor - Sense) {agent} senses a flemmardise that makes them just wait"); }
            ...
            if (ia.JustLoaded)
            {
                ...
                {
                    if (Logger.LazyInstance.LOG_WANDER_TARGET_SENSOR) { Debug.Log($"(WaitSensor - Sense) {ia.data.id} just loaded and has an existing target : {existingTarget}. We keep it."); }
                    return existingTarget;
                }
                else
                {
                    if (Logger.LazyInstance.LOG_WANDER_TARGET_SENSOR) { Debug.Log($"(WaitSensor - Sense) {ia.data.id} just loaded but has no existing target : {existingTarget}"); }
                }
            }
