...
{
    public class ClosestSpecNestSensor : LocalTargetSensorBase
    {
        ...
        {
            System.Type goal = typeof(ReturnToNestGoal);
            ...
            List<CapableData> nests = detector.FindAll<Nester>(ia.data);
            if (nests == null) { return null; }
            nests = filter_hostable_nests(ia.ID, nests);
            CapableData closest_capable = detector.ComputeClosest(ia.data, nests);
        ...
        private List<CapableData> filter_hostable_nests(string id, List<CapableData> nests)
        {
            List<CapableData> hostable_nests = new List<CapableData>();
            foreach (CapableData cdata in nests)
            {
                // we get all the capacities
                NestData nest = CapacityEngine.Instance.GetCapacityOfTypeOfCapable<NestData>(cdata);
                if (nest == null) { continue; }
                if (!nest.CanHostEntity(id)) { continue; }
                hostable_nests.Add(cdata);
            }
            return hostable_nests;
        }
    }
}