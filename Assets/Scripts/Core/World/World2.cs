...
public class World2 : Singleton<World2>
{
    [Header("World2")]
    [SerializeField] private Room2 elevator_room;
    public Level2 current_level;
    public List<Level2> loaded_levels = new List<Level2>();

    [Header("Spawn")]
    public Transform spawn_point;

    [Header("Levels")]
    public List<Level2> Levels;
    public string levels_prefab_path = "prefabs/environments/Levels/";

    [Header("Rooms")]
    public List<Room2> LoadedRooms
    {
        get
        {
    ...
    private void Start()
    {
        ...
        if (debug) { Debug.Log($"(World2) {Levels.Count} Levels: " + string.Join(", ", Levels.Select(l => l.name))); }
        ...
        if (spawn_point == null)
        {
            if (debug) { Debug.LogWarning("(World2) No spawn point found in world, please set it manually in the inspector"); }
        }
        else
        {
    ...
    private Room2 findPersoRoom(List<Room2> rooms)
    {
        ...
        string debug_message = "(World2 - findPersoRoom) Loaded rooms :\n\t";
    ...
    public void LoadLevelFromName(string level_name)
    {
        ...
        List<Level2> current_levels = loaded_levels.FindAll(l => l.name == level_name);
        ...
        Level2 level = Resources.Load<Level2>(levels_prefab_path + level_name);
        if (level == null)
        {
            if (debug) { Debug.LogError("(World2 - LoadLevelFromName) Level2 not found : " + level_name); }
            return;
        }
        ...
        load_level(level_go.GetComponent<Level2>());
    }
    private void load_level(Level2 level)
    {
        ...
        if (debug) { Debug.Log("(World2) Level2 loaded : " + level.name); }
    }
    private void load_elevator_room()
    {
        ...
        {
            elevator_room = transform.Find("Room_Elevator")?.GetComponent<Room2>();
            if (elevator_room == null && debug) { Debug.LogWarning("(World2) No elevator room found in world, please set it manually in the inspector"); }
