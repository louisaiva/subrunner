...
public class PersoInputsController : InputController
{
    ...
    private void initInputs()
    {
        ...
        dodgeCallback = ctx => { if (Capable == null) { return; } OnDodge(); };
        attackCallback = ctx => { if (Capable == null) { return; } OnAttack(); };
        talkCallback = ctx => { if (Capable == null) { return; } OnRandomTalk(); };

        // et les callbacks de conso
        useConso1Callback = ctx => { if (Capable == null) { return; } OnUseConso(1); };
        useConso2Callback = ctx => { if (Capable == null) { return; } OnUseConso(2); };
        useConso3Callback = ctx => { if (Capable == null) { return; } OnUseConso(3); };
        useConso4Callback = ctx => { if (Capable == null) { return; } OnUseConso(4); };


        // ici c'est les inputs qui prennent en charge hold input
        interactCallback = ctx => { if (Capable == null) { return; } HandleInteractInput(ctx); };

        // ici c les inputs qui ont pas besoin d'hold input
        hackCallback = ctx => { if (Capable == null) { return; } HandleRunHackInput(ctx); };
        hackMouseCallback = ctx => { if (Capable == null) { return; } handle_mouse_hack_selection(ctx); };
        exploitMouseCallback = ctx => { if (Capable == null) { return; } handle_exploit_wheel_mouse(ctx); };
        ...
        select_hackableCallback = ctx => { if (Capable == null) { return; } handle_select_hack_target_input(ctx.ReadValue<Vector2>()); };
    ...
    private void Update()
    {
        if (Capable == null || InputsDisabled) { return; }
    ...
    public void HandleRunHackInput(InputAction.CallbackContext context)
    {
        ...
        if (Controller.LazyInstance.HackableNavigator.IsSelecting)
        {
